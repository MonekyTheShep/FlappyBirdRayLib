...
void initializeGame(void) {
    Pipe pipe = {
        ...
        .position = (Vector2) {GetScreenWidth() / 2, GetScreenHeight() / 2},
    ...
    Bird bird = {
        ...
        .jumpVel = -250.0f,
        .gravVel = 200.0f,
        .position = (Vector2) {50, GetScreenWidth() / 2},
        .velocity = (Vector2) {0.0f, 0.0f}
    };
...
void updateGameMenu(GameInfo *gameInfo, MenuStates *menuState) {
    const float deltaTime = GetFrameTime();
        ...
        // Use this later
        const int touchingFloor = (bird.hitBox.y + bird.hitBox.height >= GetScreenHeight());
